function  viewer( pos_array, map, robot, handles)
%VIEWER 此处显示有关此函数的摘要
%   此处显示详细说明
axes(handles.axes1);
...
for i =1:size(map, 1) 
    ...
    for j = 1:size(map(i).adj_node, 2)
        plot_line(map(i).position, map(map(i).adj_node(j)).position, 'k',handles);
    end
...
for i=1:size(pos_array, 2)
    ...
    plot(handles.axes1, x, y, robot_color(i));%画机器人当前位置
    if(~isempty(robot(i).task))
        x_d = R*cos(alpha) + map(robot(i).task(1)).position(1);
        y_d = R*sin(alpha)+ map(robot(i).task(1)).position(2);
        plot(handles.axes1, x_d, y_d,  robot_color(i),'LineWidth',5); %画机器人目标点
    end
    
    for j = 1:size(robot(i).path, 1) - 1
        plot_line(map(robot(i).path(j, 1)).position, map(robot(i).path(j+1, 1)).position, robot_color(i),handles);
    end
...
function  plot_line( point_1, point_2 , color, handles)
x = [point_1(1), point_2(1)];
y = [point_1(2), point_2(2)];
plot(handles.axes1, x, y, color,'LineWidth',1.5);
axis equal;
hold on;
axis([-2 42 -2 42]);
end
